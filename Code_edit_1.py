...
def run_single_experiment_iteration(iteration_seed):
    ...
    dictionary_single = {'seed': iteration_seed, 'epFreq' : 1, 'targetPath': './tmp_iter_{}.csv'.format(iteration_seed), 
...
n = 25 # Number of parallel runs (simulations) to average the results over.
...
for scaling in scaling_vec:
    ...
    plt.figure(figsize=(10,6))
    plt.plot(range(len(vpi_estimate)), vpi_estimate, label=f'scaling = {scaling}')
    plt.xlabel("Episode")
    plt.ylabel("vpi")
    plt.title(f"vpi vs episode (scaling={scaling})")
    plt.legend()
    plt.grid(True)
    plt.show()
